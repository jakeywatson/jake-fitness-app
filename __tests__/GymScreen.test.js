...
describe('GymScreen — exercise display', () => {
  test('renders all 6 exercises', () => {
    render(<GymScreen appState={defaultState} dispatch={mockDispatch} isPremium={true} onUpgrade={mockOnUpgrade} />);
  ...
  test('shows Phase 2 for week 4-6', () => {
    render(<GymScreen appState={{ ...defaultState, week: 4 }} dispatch={mockDispatch} isPremium={true} onUpgrade={mockOnUpgrade} />);
    expect(screen.getByText(/PHASE 2 OF 4/)).toBeTruthy();
  });

  test('shows Phase 3 for week 7-9', () => {
    render(<GymScreen appState={{ ...defaultState, week: 7 }} dispatch={mockDispatch} isPremium={true} onUpgrade={mockOnUpgrade} />);
    expect(screen.getByText(/PHASE 3 OF 4/)).toBeTruthy();
  });

  test('shows Phase 4 for week 10-12', () => {
    render(<GymScreen appState={{ ...defaultState, week: 10 }} dispatch={mockDispatch} isPremium={true} onUpgrade={mockOnUpgrade} />);
...
describe('GymScreen — set logging', () => {
  ...
  test('shows ✓ for completed sets', () => {
    ...
    render(<GymScreen appState={state} dispatch={mockDispatch} isPremium={true} onUpgrade={mockOnUpgrade} />);
    fireEvent.press(screen.getByText('Goblet Squat'));
